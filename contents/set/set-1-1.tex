...
このとき,$a$は$A$に\textbf{属する}\index{ぞくする@属する},
$a$は$A$に\textbf{含まれる}\index{ふくまれる@含まれる}
または$A$は$a$を\textbf{含む}\index{ふくむ@含む}(\textit{$a$ is an element of $A$, $a$ is a member of $A$, $a$ belongs to $A$, $a$ is in $A$, $A$ includes $a$, $A$ contains $a$})という.
$a\in A$ でないときは $a\not\in A$ または $A\not\ni a$ で表す.
