...
\begin{exa}
    $\mathcal{O}=2^S$ は $S$ の位相の条件を満たす. この位相を \textbf{離散位相} といい, 位相空間 $(S, 2^˝)$ を \textbf{離散空間} という.
\end{exa}
...
\begin{proof}
    それぞれ開集合の定義と de Morgan の法則から直ちに導かれる. 例えば(3)は次のように示される.
...
\begin{rem*}
    開集合の補集合は閉集合であるが, 開集合でない集合が閉集合であるとは限らない. 同様に閉集合でない集合が開集合であるとは限らない.
    集合によっては開集合と閉集合のどちらでもない場合や, その両方であることもあり得る.

    例えば任意の位相空間 $(S, \mathcal{O})$ において空集合 $\emptyset$ とその台集合 $S$ は開かつ閉集合である.
    また, 離散空間 $(S, \{\emptyset, S\})$ において $\emptyset$ と $S$ を除く $S$ の任意の部分集合は閉集合でも開集合でもない.
\end{rem*}

\subsection{開核}
$(S,\mathcal{O})$ を位相空間, $A$ を $S$ の部分集合とする. $A\in$

\begin{framed}
    \begin{dfn}[]\label{dfn::inner-op}
        $(S,\mathcal{O})$ を位相空間, $A,B$ を $S$ の任意の部分集合とする. 次の条件を満たす演算 $\cdot^\circ$ を \textbf{閉核作用素} という.
